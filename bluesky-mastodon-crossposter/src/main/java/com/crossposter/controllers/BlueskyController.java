...
public class BlueskyController {
    ...
    public void handleStartOAuth(ActionEvent event) {
        ...
        try {
            // This is still the starting point for discovery, which is correct.
            String initialPdsOrigin = "https://bsky.social";
            AuthSession session = blueskyClient.startAuth(initialPdsOrigin);

            // Save the session to the registry
            ServiceRegistry.setBlueskySession(session);

            // ===== THIS IS THE FIX =====
            // The correct PDS origin was discovered during login and is stored in session.issuer
            // Use this correct value instead of the hardcoded initial one.
            ServiceRegistry.setBlueskyPdsOrigin(session.issuer);
            System.out.println("Storing verified PDS Origin for posting: " + session.issuer);
            // ===== END OF FIX =====
